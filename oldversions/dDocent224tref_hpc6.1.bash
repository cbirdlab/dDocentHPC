...
#VCFTV=$(vcftools | grep VCF | sed -e 's/VCFtools (v//' | sed -e 's/)//')  #ceb this line worked for previous versions of vcftools
	if [ "$VCFTV" != "0.1.14" ]; then   #ceb was 0.1.11
		echo "The " $VCFTV " version of VCFtools installed in your" '$PATH' "is not optimized for dDocent."
		echo "Please install only version 0.1.14"    #ceb was 0.1.11
		exit 1 
	fi
BTC=$( bedtools --version | mawk '{print $2}' | sed 's/v//g' | cut -f1,2 -d"." )
	if [ "$BTC" != "2.26" ]; then
		...
		echo "Please install only version 2.26"
		exit 1
	fi
