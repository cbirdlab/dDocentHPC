...
STACKS=$(cat namelist| parallel -j $NUMProc --no-notice "zcat {}.F.fq.gz | head -1" | mawk '!/\/1/' | wc -l)
FB1=$(( $NUMProc / 2 ))
if [ $STACKS -gt 0 ]; then
...
if [ -f "${NAMES[@]:(-1)}".R.fq.gz ]; then
	...
	STACKS=$(cat namelist| parallel -j $NUMProc --no-notice "zcat {}.R.fq.gz | head -1" | mawk '!/\/2/' | wc -l)
...
#BWA for mapping for all samples.  As of version 2.0 can handle SE or PE reads by checking for PE read files
		...
		samtools index $i-RG.bam
        	...
        	do
        	if [ -f "$i.R2.fq.gz" ]; then
        		bwa mem reference.fasta $i.R1.fq.gz $i.R2.fq.gz -L 20,5 -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.log | mawk '$6 !~/[2-9].[SH]/ && $6 !~ /[1-9][0-9].[SH]/' | samtools view -@$NUMProc -q 1 -SbT reference.fasta - > $i.bam 2>$i.bam.log
        	else
        		bwa mem reference.fasta $i.R1.fq.gz -L 20,5 -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.log | mawk '$6 !~/[2-9].[SH]/ && $6 !~ /[1-9][0-9].[SH]/' | samtools view -@$NUMProc -q 1 -SbT reference.fasta - > $i.bam 2>$i.bam.log
        	fi
        	samtools sort -@$NUMProc $i.bam -o $i.bam 
...
###New implementation of SNP calling here to save on memory	
	...
	ls mapped.*.bed | sed 's/mapped.//g' | sed 's/.bed//g' | shuf | parallel --memfree $MAXMemory -j $NUMProc --no-notice call_genos {}
####	
	...
	vcftools --vcf TotalRawSNPs.vcf $VCFGTFLAG 0.9 --out Final --counts --recode --non-ref-af 0.001 --max-non-ref-af 0.9999 --mac 1 --minQ 30 --recode-INFO-all &>VCFtools.log
fi
...
if [ -n "$1" ]; then
	main $1 | tee -a dDocent_main.LOG #Log all output
else
	main | tee -a dDocent_main.LOG  #Log all output
fi
