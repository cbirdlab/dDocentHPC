...
main(){
			...
			#Filter 2: remove reads with excessive soft clipping and orphans
				...
				if [[ "$SOFT_CLIP_CUTOFF" != "no" || "$FILTER_ORPHANS" != "no" ]]; then
					...
					SoftClipOrphanFilter(){
						echo "";echo `date` " SOFT_CLIP_CUTOFF is $2"
						if [[ "$2" != "no" && "$3" == "yes" ]]; then
	...
	if [ "$SNP" != "no" ]; then
		...
		RGBAM=$(ls *$CUTOFF.$CUTOFF2-RG.bam | wc -l)
		if [ RGBAM == 0 ]; then
			echo "";echo `date` " File Naming ERROR. Genotyping terminated.  Either rename the *.bam files to the suffix -RG.bam or filter the mapped reads in the bam files before genotyping"
		else
