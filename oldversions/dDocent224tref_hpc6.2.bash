...
main(){
			...
			if head -1 reference.$CUTOFF.$CUTOFF2.fasta | grep -e 'dDocent' reference.$CUTOFF.$CUTOFF2.fasta 1>/dev/null; then
				...
				do
					if [ -f "$i.R2.fq.gz" ]; then
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz $i.R2.fq.gz -L 30,5 -I $INSERT,$SD,$INSERTH,$INSERTL -t $NUMProc -P -a -M -T 50 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 5 -F12 -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					else
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz -L 30,5 -t $NUMProc -P -a -M -T 50 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 5 -F12 -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					fi
					echo ""
				...
				do
					if [ -f "$i.R2.fq.gz" ]; then
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz $i.R2.fq.gz -L 30,5 -t $NUMProc -P -a -M -T 50 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 5 -F12 -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					else
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz -L 30,5 -t $NUMProc -P -a -M -T 50 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 5 -F12 -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					fi
					samtools sort -@$NUMProc $i.$CUTOFF.$CUTOFF2.bam -o $i.$CUTOFF.$CUTOFF2.bam 
	...
	###New implementation of SNP calling here to save on memory	
		...
		freebayes -b split.$1.bam -t mapped.$1.bed -v raw.$1.vcf -f reference.$2.fasta -m 20 -q 20 -E 3 --min-repeat-entropy 1 --min-coverage 5 -C 2 -G 3 -F 0.025 -z 0.20 -w -a -K -p 20 --haplotype-length 125 --max-complex-gap 125 -n 0 --populations popmap.$2 
		rm split.$1.bam*
		}
		export -f call_genos
