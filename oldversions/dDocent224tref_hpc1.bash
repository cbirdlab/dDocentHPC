...
date;date;date
echo -e "\n dDocent 2.24 Hacked for HPC by cbird@tamucc.edu \n"
...
#Create list of sample names
ls $Fwild > namelist
sed -i'' -e "s/$Fsed//g" namelist
#Create an array of sample names
NUMNAMES=$(mawk '/_/' namelist | wc -l)

if [ "$NUMNAMES" -eq "$NumInd" ]; then
	NAMES=( `cat "namelist" `)
	echo " ";"The samples being processed are:"
	cat namelist | parallel "echo {}"
	echo ""
else
	echo " "
	echo "Individuals do not follow the dDocent naming convention."
...
if [ "$FixStacks" == "yes" ]; then
	...
	STACKS=$(cat namelist| parallel -j $NUMProc --no-notice "zcat {}$Fsed | head -1" | mawk '!/\/1/' | wc -l)
	FB1=$(( $NUMProc / 2 ))
	if [ $STACKS -gt 0 ]; then
	...
	if [ -f "${NAMES[@]:(-1)}"$Rsed ]; then
		...
		STACKS=$(cat namelist| parallel -j $NUMProc --no-notice "zcat {}$Rsed | head -1" | mawk '!/\/2/' | wc -l)
        ...
        if head -1 reference.$CUTOFF.$CUTOFF2.fasta | grep -e 'dDocent' reference.$CUTOFF.$CUTOFF2.fasta 1>/dev/null; then
        	...
        	do
				if [ -f "$i.R2.fq.gz" ]; then
					bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz $i.R2.fq.gz -L 20,5 -I $INSERT,$SD,$INSERTH,$INSERTL -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 1 -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
				else
					bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz -L 20,5 -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 1 -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
				fi
				echo ""
        	...
        	do
				if [ -f "$i.R2.fq.gz" ]; then
					bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz $i.R2.fq.gz -L 20,5 -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 1 -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
				else
					bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz -L 20,5 -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 1 -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
				fi
				samtools sort -@$NUMProc $i.$CUTOFF.$CUTOFF2.bam -o $i.$CUTOFF.$CUTOFF2.bam 
...
###New implementation of SNP calling here to save on memory	
	...
	call_genos(){
	samtools view -@$FB2 -b -1 -L mapped.$1.bed -o split.$1.bam cat.$2-RRG.bam
	samtools index split.$1.bam
	freebayes -b split.$1.bam -t mapped.$1.bed -v raw.$1.vcf -f reference.$2.fasta -m 5 -q 5 -E 3 --min-repeat-entropy 1 -V --populations popmap.$2 -n 10
	rm split.$1.bam*
	}
	export -f call_genos

	echo "";echo `date` " Using GNU Parallel & FreeBayes to call SNPs"
	#this line executes the call_genos function and passes 2 variables to it
	ls mapped.*.$CUTOFF.$CUTOFF2.bed | sed 's/mapped.//g' | sed 's/.bed//g' | shuf | parallel --progress --memfree $MAXMemory -j $NUMProc --no-notice call_genos {} $CUTOFF.$CUTOFF2
####	
	...
	vcftools --vcf TotalRawSNPs.$CUTOFF.$CUTOFF2.vcf --geno 0.9 --out Final --counts --recode --non-ref-af 0.001 --max-non-ref-af 0.9999 --mac 1 --minQ 30 --recode-INFO-all &>VCFtools.$CUTOFF.$CUTOFF2.log
...
if [ -f $i.$R.fq.gz ]; then
	...
	java -jar $TRIMMOMATIC PE -threads $NUMProc -phred33 $i.F.fq.gz $i.R.fq.gz $i._r1.fq.gz $i.unpairedF.fq.gz $i.r2.fq.gz $i.unpairedR.fq.gz ILLUMINACLIP:$ADAPTERS:2:30:10 TRAILING:10 SLIDINGWINDOW:20:10 CROP:104 MINLEN:104  &> $i.pe.trim.log
...
else java -jar $TRIMMOMATIC SE -threads $NUMProc -phred33 $i.F.fq.gz $i.r1.fq.gz ILLUMINACLIP:$ADAPTERS:2:30:10 TRAILING:10 SLIDINGWINDOW:20:10 CROP:104 MINLEN:104 &> $i.trim.log
fi 
done
...
#echo "Trimming Sample $i"
if [ -f $i.R.fq.gz ]; then
java -jar $TRIMMOMATIC PE -threads $NUMProc -phred33 $i.F.fq.gz $i.R.fq.gz $i.R1.fq.gz $i.unpairedF.fq.gz $i.R2.fq.gz $i.unpairedR.fq.gz ILLUMINACLIP:$ADAPTERS:2:30:10 LEADING:15 TRAILING:15 SLIDINGWINDOW:10:10 MINLEN:75 &> $i.trim.log
else java -jar $TRIMMOMATIC SE -threads $NUMProc -phred33 $i.F.fq.gz $i.R1.fq.gz ILLUMINACLIP:$ADAPTERS:2:30:10 LEADING:15 TRAILING:15 SLIDINGWINDOW:105:10 MINLEN:75 &> $i.trim.log
fi 
done
...
#this block of code will either align or concatenate r1 & r2 seqs and create a file of unique sequences 
	echo " "
	echo "Reference Genome Assembly Step 1, Select Cutoffs"
	date


	if [ ${NAMES[@]:(-1)}.r1.fq.gz -nt ${NAMES[@]:(-1)}.uniq.seqs ];then
		echo " "
		echo "the *.fq.gz files are newer than the *uniq.seqs files or the *.uniq.seq files do not exist"
		
		if [ ! -s ${NAMES[@]:(-1)}.uniq.seqs ];then
			echo "make *.uniq.seqs files"
			date		
			if [[ "$ATYPE" == "PE" || "$ATYPE" == "RPE" ]]; then
			#If PE assembly, creates a concatenated file of every unique for each individual in parallel
				cat namelist | parallel --no-notice -j $NUMProc "zcat {}.r1.fq.gz | mawk '$AWK1' | mawk '$AWK2' > {}.forward"
				cat namelist | parallel --no-notice -j $NUMProc "zcat {}.r2.fq.gz | mawk '$AWK1' | mawk '$AWK2' > {}.reverse"
				if [ "$ATYPE" = "RPE" ]; then
					echo "RPE assembly"
			...
			if [ "$ATYPE" == "SE" ]; then
				...
				cat namelist | parallel --no-notice -j $NUMProc "zcat {}.r1.fq.gz | mawk '$AWK1' | mawk '$AWK2' | perl -e '$PERLT' > {}.uniq.seqs"
			fi
			if [ "$ATYPE" == "OL" ]; then
			...
			echo "***************************************************************************"
			echo "if you want to recreate *uniq.seqs files then you need to delete the present *.uniq.seq files "
			echo "***************************************************************************"
			echo " "
		fi
	fi
...
if [ -s "uniq.seqs.gz" ]; then
	echo " "
	echo "uniq.seqs.gz exists"
	if [ uniq.seqs.gz -nt uniq.seqs ]; then
		echo "unzip uniq.seqs.gz"
			date
...
if [ ! -s "uniqseq.data" ]; then
	echo " "
	echo "make uniqseq.data"
	date
	seq 2 2 20 > pfile
...
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
echo "CEB has modified dDocent here such that there is no user input"
echo "set CUTOFFs in the config file for this to keep running"
...
if [ ! -s "uniqseq.$CUTOFF.peri.data" ];then
	echo " "
	echo "make uniqseq.$CUTOFF.peri.data"
		date
	for ((i = 2; i <= $NUM; i++)); do
		echo $i >> ufile
	done
...
if [ "$HPC" = "yes" ]; then
	echo "Stopping because config file is set to HPC, Get Graphs for cutoffs, then stop? yes"
	exit
fi
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
if [[ "$ATYPE" == "PE" || "$ATYPE" == "RPE" ]]; then
	...
	date
	sed -e 's/NNNNNNNNNN/	/g' uniq.$CUTOFF.$CUTOFF2.fasta | cut -f1 > uniq.$CUTOFF.$CUTOFF2.F.fasta
	...
	sort -k2,2 -g contig.cluster.totaluniqseq.$CUTOFF.$CUTOFF2 | sed -e 's/NNNNNNNNNN/	/g' > rcluster.$CUTOFF.$CUTOFF2
	...
	date
	rainbow merge -o rbasm.$CUTOFF.$CUTOFF2.out -a -i rbdiv.$CUTOFF.$CUTOFF2.out -r 2 -N10000 -R10000 -l 20 -f 0.75
	echo ""
	...
	sed -e 's/NNNNNNNNNN/	/g' rainbow.$CUTOFF.$CUTOFF2.fasta | cut -f1 | gawk 'BEGIN {RS = ">" ; FS = "\n"} NR > 1 {print "@"$1"\n"$2"\n+""\n"gensub(/./, "I", "g", $2)}' > ref.$CUTOFF.$CUTOFF2.F.fq
	sed -e 's/NNNNNNNNNN/	/g' rainbow.$CUTOFF.$CUTOFF2.fasta | cut -f2 | gawk 'BEGIN {RS = ">" ; FS = "\n"} NR > 1 {print "@"$1"\n"$2"\n+""\n"gensub(/./, "I", "g", $2)}' > ref.$CUTOFF.$CUTOFF2.R.fq
	...
	pearRM -f ref.$CUTOFF.$CUTOFF2.F.fq -r ref.$CUTOFF.$CUTOFF2.R.fq -o overlap.$CUTOFF.$CUTOFF2 -p 0.001 -j $NUMProc -n $LENGTH
	...
	paste other.$CUTOFF.$CUTOFF2.F other.$CUTOFF.$CUTOFF2.R | mawk '{if ($1 ~ />/) print $1; else print $0}' | sed 's/	/NNNNNNNNNN/g' > other.$CUTOFF.$CUTOFF2.FR
