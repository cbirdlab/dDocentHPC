...
main(){
			...
			if head -1 reference.$CUTOFF.$CUTOFF2.fasta | grep -e 'dDocent' reference.$CUTOFF.$CUTOFF2.fasta 1>/dev/null; then
				...
				do
					if [ -f "$i.R2.fq.gz" ]; then
						...
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz $i.R2.fq.gz -L $MAPPING_CLIPPING_PENALTY -I $INSERT,$SD,$INSERTH,$INSERTL -t $NUMProc -a -M -T $MAPPING_MIN_ALIGNMENT_SCORE -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | samtools view -@$NUMProc -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
						
					else
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz -L $MAPPING_CLIPPING_PENALTY -t $NUMProc -a -M -T $MAPPING_MIN_ALIGNMENT_SCORE -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | samtools view -@$NUMProc -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					fi
					echo ""
					echo `date` " run samtools sort" $i
					samtools sort -@$NUMProc $i.$CUTOFF.$CUTOFF2.bam -o $i.$CUTOFF.$CUTOFF2.bam 
					mv $i.$CUTOFF.$CUTOFF2.bam $i.$CUTOFF.$CUTOFF2-RAW.bam
					echo ""
					echo `date` " run samtools index" $i
					#samtools index $i.$CUTOFF.$CUTOFF2.bam
				done
			else
		...
		if [ "$ATYPE" == "PE" ]; then 	#paired end alignments
			#Filter 1: remove reads based on samtools flags
				...
				BITScustom=$(($SAMTOOLS_VIEW_Fcustom+$SAMTOOLS_VIEW_fcustom))
					...
					if [[ "$BITS" != "0" && "$BITScustom" != "0" ]]; then
						ls *$CUTOFF.$CUTOFF2-RAW.bam | sed 's/\-RAW.bam//g' | parallel --no-notice "samtools view -h -q $MAPPING_MIN_QUALITY -F $SAMTOOLS_VIEW_F -f $SAMTOOLS_VIEW_f2 {}-RAW.bam | samtools view -Sh1 -F $SAMTOOLS_VIEW_Fcustom -f SAMTOOLS_VIEW_fcustom - -o {}-RG.bam "
					elif [[ "$BITS" != 0 && "$BITScustom" == 0 ]]; then
						ls *$CUTOFF.$CUTOFF2-RAW.bam | sed 's/\-RAW.bam//g' | parallel --no-notice "samtools view -h1 -q $MAPPING_MIN_QUALITY -F $SAMTOOLS_VIEW_F -f SAMTOOLS_VIEW_f2 {}-RAW.bam -o {}-RG.bam "
					elif [[ "$BITS" == 0 && "$BITScustom" != 0 ]]; then
						ls *$CUTOFF.$CUTOFF2-RAW.bam | sed 's/\-RAW.bam//g' | parallel --no-notice "samtools view -h1 -q $MAPPING_MIN_QUALITY -F $SAMTOOLS_VIEW_Fcustom -f SAMTOOLS_VIEW_fcustom {}-RAW.bam -o {}-RG.bam "
					elif [[ "$BITScustom" == 0 && "$BITS" == 0 ]]; then
						ls *$CUTOFF.$CUTOFF2-RAW.bam | sed 's/\-RAW.bam//g' | parallel --no-notice "samtools view -h1 -q $MAPPING_MIN_QUALITY {}-RAW.bam -o {}-RG.bam "
					fi
				#else
			...
			#Filter 2: remove reads with excessive soft clipping and orphans
				if [[ "$SOFT_CLIP_CUTOFF" != "no" || "$FILTER_ORPHANS" != "no" ]]; then
					...
					ls *$CUTOFF.$CUTOFF2-RG.bam | parallel --no-notice "SoftClipOrphanFilter {} $SOFT_CLIP_CUTOFF $FILTER_ORPHANS reference.$CUTOFF.$CUTOFF2.fasta $FILTER_MIN_AS"
				fi
			
			#Index the filtered bam files 
				echo "";echo "  "`date` " Indexing the filtered BAM files"
				ls *$CUTOFF.$CUTOFF2-RG.bam | parallel --no-notice "samtools index {}" 
	...
	# if [ "$MAP" != "no" ]; then
		# echo "";echo `date` " Create Intervals - No Mapping"
		# RGBAM=$(ls *$CUTOFF.$CUTOFF2-RG.bam | wc -l)
		# if [ RGBAM > 0 ]; then
			# ls *.$CUTOFF.$CUTOFF2-RG.bam >bamlist.$CUTOFF.$CUTOFF2.list
			# CreateIntervals 
		# else
			# echo "";echo `date` " POSSIBLE ERROR: Before genotyping either rename the *.bam files to the suffix -RG.bam or filter the mapped reads in the bam files"
		# fi
	# fi
	...
	if [ "$SNP" != "no" ]; then
		...
		if [ RGBAM == 0 ]; then
			...
			if [[ "$MAP" == "no" && -f "map.$CUTOFF.$CUTOFF2.bed" ]]; then
				echo "  "`date` " Recreating intervals 2..."
				CreateIntervals 
			fi
			...
			#ceb inserting this to separate splitting of bam files from freebayes routine
			#ceb  this is too slow
			split_bam(){
			...
			ls mapped.*.$CUTOFF.$CUTOFF2.bed | sed 's/mapped.//g' | sed 's/.bed//g' | shuf | parallel --no-notice -j $NUMProc "split_bam {} $CUTOFF.$CUTOFF2 $FB2"
			
			if [ "$POOLS" == "no" ]; then
				echo; echo " "`date` " Genotyping individuals of ploidy $PLOIDY using freebayes..."			
				ls mapped.*.$CUTOFF.$CUTOFF2.bed | sed 's/mapped.//g' | sed 's/.bed//g' | shuf | parallel --no-notice -j $NUMProc "freebayes -b split.{}.bam -t mapped.{}.bed -v raw.{}.vcf -f reference.$CUTOFF.$CUTOFF2.fasta -V -p $PLOIDY -n $BEST_N_ALLELES -m $MIN_MAPPING_QUAL -q $MIN_BASE_QUAL -E $HAPLOTYPE_LENGTH --min-repeat-entropy $MIN_REPEAT_ENTROPY --min-coverage $MIN_COVERAGE -F $MIN_ALT_FRACTION -C $MIN_ALT_COUNT -G $MIN_ALT_TOTAL -z $READ_MAX_MISMATCH_FRACTION -Q $FREEBAYES_Q -U $FREEBAYES_U --populations popmap.$CUTOFF.$CUTOFF2 "
			...
			# #if $POOLS == true and a pool_ploidy file is provided then
				...
				ls mapped.*.$CUTOFF.$CUTOFF2.bed | sed 's/mapped.//g' | sed 's/.bed//g' | shuf | parallel --no-notice "freebayes -b split.{}.bam -t mapped.{}.bed -v raw.{}.vcf -f reference.$CUTOFF.$CUTOFF2.fasta -J -w -a -V -n $BEST_N_ALLELES -m $MIN_MAPPING_QUAL -q $MIN_BASE_QUAL -E $HAPLOTYPE_LENGTH --min-repeat-entropy $MIN_REPEAT_ENTROPY --min-coverage $MIN_COVERAGE -F $MIN_ALT_FRACTION -C $MIN_ALT_COUNT -G $MIN_ALT_TOTAL -z $READ_MAX_MISMATCH_FRACTION -Q $FREEBAYES_Q -U $FREEBAYES_U --populations popmap.$CUTOFF.$CUTOFF2 --cnv-map $POOL_PLOIDY_FILE " 
