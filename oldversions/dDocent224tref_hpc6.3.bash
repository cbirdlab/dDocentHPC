...
if [ -n "$1" ]; then
	...
	POOL_PLOIDY_FILE=$(grep -A1 '^If the data is pooled, will you provide a file with the ploidy of each pool' $CONFIG | tail -1)
...
main(){
			...
			if head -1 reference.$CUTOFF.$CUTOFF2.fasta | grep -e 'dDocent' reference.$CUTOFF.$CUTOFF2.fasta 1>/dev/null; then
				...
				do
					if [ -f "$i.R2.fq.gz" ]; then
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz $i.R2.fq.gz -L $MAPPING_CLIPPING_PENALTY -I $INSERT,$SD,$INSERTH,$INSERTL -t $NUMProc -P -a -M -T $MAPPING_MIN_ALIGNMENT_SCORE -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q $MAPPING_MIN_QUALITY -F $SAMTOOLS_VIEW_F -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					else
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz -L $MAPPING_CLIPPING_PENALTY -t $NUMProc -P -a -M -T $MAPPING_MIN_ALIGNMENT_SCORE -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q $MAPPING_MIN_QUALITY -F $SAMTOOLS_VIEW_F -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					fi
					echo ""
				...
				do
					if [ -f "$i.R2.fq.gz" ]; then
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz $i.R2.fq.gz -L $MAPPING_CLIPPING_PENALTY -t $NUMProc -P -a -M -T $MAPPING_MIN_ALIGNMENT_SCORE -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q $MAPPING_MIN_QUALITY -F $SAMTOOLS_VIEW_F -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					else
						bwa mem reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz -L $MAPPING_CLIPPING_PENALTY -t $NUMProc -P -a -M -T $MAPPING_MIN_ALIGNMENT_SCORE -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q $MAPPING_MIN_QUALITY -F $SAMTOOLS_VIEW_F -SbT reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					fi
					samtools sort -@$NUMProc $i.$CUTOFF.$CUTOFF2.bam -o $i.$CUTOFF.$CUTOFF2.bam 
	...
	###New implementation of SNP calling here to save on memory	
		...
		call_genos(){
			...
			freebayes -b split.$1.bam -t mapped.$1.bed -v raw.$1.vcf -f reference.$2.fasta -V -n $BEST_N_ALLELES -m $MIN_MAPPING_QUAL -q $MIN_BASE_QUAL -E $HAPLOTYPE_LENGTH --min-repeat-entropy $MIN_REPEAT_ENTROPY --min-coverage $MIN_COVERAGE -F $MIN_ALT_FRACTION -C $MIN_ALT_COUNT -G $MIN_ALT_TOTAL -z $READ_MAX_MISMATCH_FRACTION --populations popmap.$2 
			rm split.$1.bam*
		}
		export -f call_genos
		...
		call_allelotypes(){
			...
			if [ "$POOL_PLOIDY_FILE" == "no" ]; then
				freebayes -b split.$1.bam -t mapped.$1.bed -v raw.$1.vcf -f reference.$2.fasta -K -w -a -V -n $BEST_N_ALLELES -m $MIN_MAPPING_QUAL -q $MIN_BASE_QUAL -E $HAPLOTYPE_LENGTH --min-repeat-entropy $MIN_REPEAT_ENTROPY --min-coverage $MIN_COVERAGE -F $MIN_ALT_FRACTION -C $MIN_ALT_COUNT -G $MIN_ALT_TOTAL -z $READ_MAX_MISMATCH_FRACTION --populations popmap.$2 
				...
				freebayes -b split.$1.bam -t mapped.$1.bed -v raw.$1.vcf -f reference.$2.fasta -J -w -a -V --cnv-map $POOL_PLOIDY_FILE -n $BEST_N_ALLELES -m $MIN_MAPPING_QUAL -q $MIN_BASE_QUAL -E $HAPLOTYPE_LENGTH --min-repeat-entropy $MIN_REPEAT_ENTROPY --min-coverage $MIN_COVERAGE -F $MIN_ALT_FRACTION -C $MIN_ALT_COUNT -G $MIN_ALT_TOTAL -z $READ_MAX_MISMATCH_FRACTION --populations popmap.$2 
																													#-m 20 -q 20 --min-repeat-entropy 1 --min-coverage 5 -C 2 -G 3 -F 0.025 -z 0.20 -w -a -K -p 20 --haplotype-length 125 -n 0 --populations popmap.$2 	
			fi
			rm split.$1.bam*
		}
		...
		#CEB elseif config file specifies pools then...
			...
			#this line executes the call_genos function and passes 2 variables to it
			ls mapped.*.$CUTOFF.$CUTOFF2.bed | sed 's/mapped.//g' | sed 's/.bed//g' | shuf | parallel --progress --memfree $MAXMemory -j $NUMProc --no-notice call_allelotypes {} $CUTOFF.$CUTOFF2
		fi
