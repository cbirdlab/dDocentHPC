...
main(){
	...
	if [ "$SNP" != "no" ]; then
		...
		###New implementation of SNP calling here to save on memory	
			...
			ls mapped.*.$CUTOFF.$CUTOFF2.bed | sed 's/mapped.//g' | sed 's/.bed//g' | shuf | parallel --no-notice "split_bam {} $CUTOFF.$CUTOFF2 $FB2 $POOL_PLOIDY_FILE $BEST_N_ALLELES $MIN_MAPPING_QUAL $MIN_BASE_QUAL $HAPLOTYPE_LENGTH $MIN_REPEAT_ENTROPY $MIN_COVERAGE $MIN_ALT_FRACTION $MIN_ALT_COUNT $MIN_ALT_TOTAL $READ_MAX_MISMATCH_FRACTION $POOLS $PLOIDY "
