...
if [ -n "$1" ]; then
	...
	REFDIR=$(grep -A1 Reference $CONFIG | tail -1)
...
main(){
	...
	echo "";echo `date` " End extensions selected"
	#$$$$$$$$$$$$$$$$$$$$$$$$$$$CEB$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
	...
	if [ "$MAP" != "no" ]; then
		...
		if [ $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta -nt $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta.fai ]; then
			samtools faidx $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta
			bwa index $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta &> index.$CUTOFF.$CUTOFF2.log
		fi
	...
	#and if so, modifies the expected insert length distribution for BWA's metric for proper pairing
			...
			if head -1 $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta | grep -e 'dDocent' $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta 1>/dev/null; then
				...
				do
					if [ -f "$i.R2.fq.gz" ]; then
						bwa mem $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz $i.R2.fq.gz -L 20,5 -I $INSERT,$SD,$INSERTH,$INSERTL -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 1 -SbT $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					else
						bwa mem $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz -L 20,5 -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 1 -SbT $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					fi
					echo ""
				...
				do
					if [ -f "$i.R2.fq.gz" ]; then
						bwa mem $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz $i.R2.fq.gz -L 20,5 -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 1 -SbT $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					else
						bwa mem $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta $i.R1.fq.gz -L 20,5 -t $NUMProc -a -M -T 10 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.$CUTOFF.$CUTOFF2.log | mawk '!/\t[2-9].[SH].*/' | mawk '!/[2-9].[SH]\t/' | samtools view -@$NUMProc -q 1 -SbT $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta - > $i.$CUTOFF.$CUTOFF2.bam 2>$i.$CUTOFF.$CUTOFF2.bam.log
					fi
					samtools sort -@$NUMProc $i.$CUTOFF.$CUTOFF2.bam -o $i.$CUTOFF.$CUTOFF2.bam 
		...
		if [ ! -s ${NAMES[@]:(-1)}.uniq.seqs ];then
			...
			#If PE assembly, creates a concatenated file of every unique for each individual in parallel
				...
				if [ "$ATYPE" = "RPE" ]; then
					...
					cat namelist.$CUTOFF.$CUTOFF2 | parallel --no-notice -j $NUMProc "paste -d '-' {}.forward {}.reverse | mawk '$AWK3'| sed 's/-/NNNNNNNNNN/' | sort | uniq -c -w $FRL| sed -e '$SED1' | sed -e '$SED2' > {}.uniq.seqs"
				else
					echo "";echo `date` " PE assembly"
					cat namelist.$CUTOFF.$CUTOFF2 | parallel --no-notice -j $NUMProc "paste -d '-' {}.forward {}.reverse | mawk '$AWK3'| sed 's/-/NNNNNNNNNN/' | perl -e '$PERLT' > {}.uniq.seqs"
				fi
				rm *.forward
				rm *.reverse
			fi
...
if [[ "$ATYPE" != "PE" && "$ATYPE" != "RPE" ]]; then
	...
	date
cd-hit-est -i totalover.$CUTOFF.$CUTOFF2.fasta -o $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta.original -M 0 -T 0 -c $simC

echo ""
echo -n "sed command "
date
sed -e 's/^C/NC/g' -e 's/^A/NA/g' -e 's/^G/NG/g' -e 's/^T/NT/g' -e 's/T$/TN/g' -e 's/A$/AN/g' -e 's/C$/CN/g' -e 's/G$/GN/g' $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta.original > $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta
...
date
bwa index $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta
	echo ""
...
if [ "$ASSEMBLY" == "no" ]; then
        echo -e "\nReference contigs need to be in a file named $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta\n"
        sleep 1
else
...
if [ -n "$1" ]; then
	#main $1
	main $NUMProc $MAXMemory $TRIM $FixStacks $ASSEMBLY $ATYPE $simC $MAP $optA $optB $optO $SNP $MAIL $HPC $MANCUTOFF $CUTOFF $CUTOFF2
else
	#main
	echo ""; echo `date` " This is the HPC version of dDocent.  A config file must be specified"
	echo ""; echo `date` " Aborting dDocent 4 HPC"
...
date
gzip -f concat.fasta concat.seq rcluster.$CUTOFF.$CUTOFF2 rbdiv.$CUTOFF.$CUTOFF2.out rbasm.$CUTOFF.$CUTOFF2.out rainbow.$CUTOFF.$CUTOFF2.fasta $REFDIR/reference.$CUTOFF.$CUTOFF2.fasta.original uniq.seqs uniq.$CUTOFF.$CUTOFF2.fasta totaluniqseq.$CUTOFF.$CUTOFF2 uniq..$CUTOFF.$CUTOFF2F.fasta uniq.$CUTOFF.$CUTOFF2.RC.fasta 2> /dev/null &
###############################################################################################
