...
##demultiplexFQ, trimFQ, mkREF, mkBAM, fltrBAM, mkfltrBAM, mkVCF	
#if [ -n "$3" ]; then
...
# Save the first argument to the variable FUNKTION
FUNKTION=$1

# do not use files created by previous runs
OVERWRITE=TRUE
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    # If the --preserve or -p option is specified, set OVERWRITE to TRUE
    -p | --preserve)
		OVERWRITE=FALSE
		shift
		;;
	# If the -d or --skip_dependency_check option is specified, save the next argument to refGENOME
    -d | --skip_dependency_check)
		DEPENDENCIES=FALSE
		shift
		;;
	# If the -1 or --read1fq option is specified, save the next argument to fq1FilePATTERN
    -1 | --fq1)
		fq1FilePATTERN="$2"
		shift
		shift
		;;
	# If the -1 or --read1fq option is specified, save the next argument to fq2FilePATTERN
    -2 | --fq2)
		fq2FilePATTERN="$2"
		shift
		shift
		;;
    # If the -b or --bam option is specified, save the next argument to bamFilePATTERN
    -b | --bam)
		bamFilePATTERN="$2"
		shift
		shift
		;;
    # If the -c or --configfile option is specified, save the next argument to CONFIG
    -c | --configfile)
		CONFIG="$2"
		shift
		shift
		;;
	# If the -r or --referencegenome option is specified, save the next argument to refGENOME
    -r | --referencegenome)
		refGENOME="$2"
		shift
		shift
...
checkCliINPUT(){
	...
	# If FUNKTION is not set, output an error message and exit the script
	case "$FUNKTION" in
		trimFQ|trimFQref|trimFQmap|mkREF|mkBAM|fltrBAM|mkfltrBAM|mkVCF)
			# code to execute if FUNKTION is one of the allowed values
			;;
		*)
			echo "Error: the first argument must be one of the following values: trimFQ, mkREF, mkBAM, fltrBAM, mkfltrBAM, mkVCF"
			exit 1
			;;
	esac

	# If CONFIG is not set, output an error message and exit the script
	if [[ -z "$CONFIG" ]]; then
	  echo "Error: A config file must be specified with -c | --configfile"
	  exit 1
	fi
	if [ ! -f $CONFIG ]; then
		echo " ";echo `date` "  ERROR: Config file not found."
		exit 1
	else
		echo " ";echo `date` "  Config file found."
	fi

	case "$FUNKTION" in
		mkBAM|fltrBAM|mkfltrBAM|mkVCF)
			# code to execute if FUNKTION is one of the allowed values
			# If bamFilePATTERN is not set, output an error message and exit the script
			if [[ -z "$bamFilePATTERN" && $FUNKTION ==]]; then
			  echo "Error: bam input file pattern must be specified with -b | --bam"
			  exit 1
			else
				echo;echo -e "Input BAM Files:\n$(ls $bamFilePATTERN)"
			fi
			
			# If refGENOME is not set, output an error message and exit the script
			if [[ -z "$refGENOME" ]]; then
			  echo "Error: A reference genome file must be specified with -r | --referencegenome"
			  exit 1
			fi
			if [ ! -f $refGENOME ]; then
				echo " ";echo `date` "  ERROR: Reference genome not found."
				exit 1
			else
				echo " ";echo `date` "  Reference genome found."
			fi
			;;
	esac
...
if [[ $DEPENDENCIES == TRUE ]]; then
	echo
	echo `date` "Checking for all required software..."
...
###############################################################################################
#function to return the common suffix in a file with one word (ie filename) per row
###############################################################################################
commonSUFFIX(){
	awk '
		{
...
getNAMES(){
	...
	if [ "$numNAMES" == "$NumInd" ]; then
		...
		echo "  Please rename individuals to: Population_Individual.F.fq.gz"
...
MAIN(){
	...
	if [ "$FUNKTION" == "trimFQ" ]; then
		...
		Rsed=".R2.fq.gz"
		echo "";echo "  extensions selected: $Fwild $Rwild"
		inFileEXT=$(ls $inFilePATTERN | commonSUFFIX)
	...
	if [ "$FUNKTION" == "fltrBAM" ]; then
		...
		NumInd=$(ls $bamFilePATTERN | wc -l)

		#Create list of sample names
		bamFileEXT=$(ls $bamFilePATTERN | commonSUFFIX)
		#fix the bamFileEXT variable to get the base
		outFileEXT=$(echo ${bamFileEXT*/%} | sed -e '/\-RAW$//' -e '/$/\-RG.bam' ) 
		bamFileExtPATTERN=$(echo $bamFileEXT | sed -e 's/\./\\\./g' -e 's/\-/\\\-/g')
		# do we really need the names printed to a file????
		ls $bamFilePATTERN | sed -e "s/${bamFileExtPATTERN}//" > namelist
		NAMES=($(ls $bamFilePATTERN | sed -e "s/${fileExtPATTERN}//"))
	elif [ "$FUNKTION" == "mkBAM" ]; then
		# get the file extension which should follow .F.fq.gz or .R.fq.gz similar F|1|R1|r1, R|2|R2|r2
		fq1FileEXT=$(ls $fq1FilePATTERN | commonSUFFIX | sed "s/^.*\(\.(r1|R1|F|f|1)\.f.*q.*$\)/\1/")
		fq2FileEXT=$(ls $fq2FilePATTERN | commonSUFFIX | sed "s/^.*\(\.(r2|R2|R|r|2)\.f.*q.*$\)/\1/")
		F=$(echo $fq1FileEXT | sed "/^.*\.\((r1|R1|F|f|1)\)\.f.*q.*$/\1/")
		Fwild=$(echo "*${fq1FileEXT}")
		Fsed=$fq1FileEXT
		R=$(echo $fq2FileEXT | sed "/^.*\.\((r2|R2|R|r|2)\)\.f.*q.*$/\1/")
		Rwild=$(echo "*${fq2FileEXT}")
		Rsed=$fq2FileEXT
		
		fqiFileExtPATTERN=$(echo $fq1FileEXT | sed -e 's/\./\\\./g' -e 's/\-/\\\-/g')
...
function FILTERBAM2(){
	...
	if [[ "$SOFT_CLIP_CUT" != "no" && "$FILTER_ORPHANS" != "no" ]]; then
		...
		echo ${NAMES[@]} | \
			tr " " "\n" | \
			parallel --no-notice --env _ -j $NUMProc "filterSamFLAGS {}${bamFileEXT} $MAPPING_MIN_QUALITY $SAMTOOLS_VIEW_F $SAMTOOLS_VIEW_f2 $SAMTOOLS_VIEW_Fcustom $SAMTOOLS_VIEW_fcustom | filterAS $FILTER_MIN_AS $FILTER_MIN_AS_LEN | filterSoftClipTOTAL $SOFT_CLIP_CUT | filterORPHANS | samtools view -b -o {}$outFileEXT "
	elif [[ "$SOFT_CLIP_CUT" == "no" && "$FILTER_ORPHANS" != "no" ]]; then
		echo "";echo "  "`date` " Filtering bam files by Samtools Flags, Alignment Scores relative to Read Length, and Orphans..."
		echo ${NAMES[@]} | \
			tr " " "\n" | \
			parallel --no-notice --env _ -j $NUMProc "filterSamFLAGS {}${bamFileEXT} $MAPPING_MIN_QUALITY $SAMTOOLS_VIEW_F $SAMTOOLS_VIEW_f2 $SAMTOOLS_VIEW_Fcustom $SAMTOOLS_VIEW_fcustom | filterAS $FILTER_MIN_AS $FILTER_MIN_AS_LEN | filterORPHANS | samtools view -b -o {}$outFileEXT "
	elif [[ "$SOFT_CLIP_CUT" != "no" && "$FILTER_ORPHANS" == "no" ]]; then
		echo "";echo "  "`date` " Filtering bam files by Samtools Flags, Soft Clipped Bases, and Alignment Scores relative to Read Length..."
		echo ${NAMES[@]} | \
			tr " " "\n" | \
			parallel --no-notice --env _ -j $NUMProc "filterSamFLAGS {}${bamFileEXT} $MAPPING_MIN_QUALITY $SAMTOOLS_VIEW_F $SAMTOOLS_VIEW_f2 $SAMTOOLS_VIEW_Fcustom $SAMTOOLS_VIEW_fcustom | filterAS $FILTER_MIN_AS $FILTER_MIN_AS_LEN | filterSoftClipTOTAL $SOFT_CLIP_CUT | samtools view -b -o {}$outFileEXT "
	elif [[ "$SOFT_CLIP_CUT" == "no" && "$FILTER_ORPHANS" == "no" ]]; then
		echo "";echo "  "`date` " Filtering bam files by Samtools Flags and Alignment Scores relative to Read Length..."
		echo ${NAMES[@]} | \
			tr " " "\n" | \
			parallel --no-notice --env _ -j $NUMProc "filterSamFLAGS {}${bamFileEXT} $MAPPING_MIN_QUALITY $SAMTOOLS_VIEW_F $SAMTOOLS_VIEW_f2 $SAMTOOLS_VIEW_Fcustom $SAMTOOLS_VIEW_fcustom | filterAS $FILTER_MIN_AS $FILTER_MIN_AS_LEN | samtools view -b -o {}$outFileEXT "
	fi
	...
	echo ${NAMES[@]} | \
			tr " " "\n" | \
			parallel --no-notice -j $NUMProc "samtools index {}$outFileEXT" 
	# ls *$CUTOFFS-RG.bam | parallel --no-notice -j $NUMProc "samtools index {}" 
...
gzip -f concat.fasta \
	concat.seq \
	rcluster.$CUTOFF.$CUTOFF2 \
	rbdiv.$CUTOFF.$CUTOFF2.out \
	rbasm.$CUTOFF.$CUTOFF2.out \
	rainbow.$CUTOFF.$CUTOFF2.fasta \
	reference.$CUTOFF.$CUTOFF2.fasta.original \
	uniq.seqs uniq.$CUTOFF.$CUTOFF2.fasta \
	totaluniqseq.$CUTOFF.$CUTOFF2 \
	uniq..$CUTOFF.$CUTOFF2F.fasta \
	uniq.$CUTOFF.$CUTOFF2.RC.fasta 2> /dev/null &
###############################################################################################
